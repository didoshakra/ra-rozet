...
//   p_StripedRows, //Смугасті ряди
  ...
  const preparedTableData = useMemo(() => {
    ...
    const temp = initialData.map((data, idx) => {
      ...
      //   tempData._selected = true; // Set new field/Встановити нове поле
  ...
  const onChangeSearch = (e) => {
    ...
    // console.log("FlowbiteUI.js/onChangeSearch=", seachData);
    // console.log("FlowbiteUI.js/onChangeSearch=", seachData);
    // }
    // }
  };
  ...
  //Selected / Записємо селект(true/false) в _selected роточого масиву(tableData)
  const selectRows = (e) => {
    ...
    //   nRow,
    // );
    let cTableData = [...tableData]; //Копія робочого масиву обєктів
    cTableData[nRow] = { ...cTableData[nRow], ...{ _selected: newSelect } }; //Якщо міняєм не всі поля в об'єкті
    setTableData(cTableData);
    //--------------------------------------------------------------
  };
